...
moran_ROI_nearest = pickle.load(open(r'C:\Users\torstsl\Projects\axon2pmini\results\moran_ROI_nearest.pickle','rb'))
...
for key in moran_ROI_nearest.keys():
    for NAT in NATs:
        
        results[counter,0] = moran_ROI_nearest[key]['moran'][NAT].I
        results[counter,1] = moran_ROI_nearest[key]['moran'][NAT].EI
        results[counter,2] = np.percentile(moran_ROI_nearest[key]['moran'][NAT].sim, 95)
        results[counter,3] = moran_ROI_nearest[key]['moran'][NAT].p_rand
        results[counter,4] = moran_ROI_nearest[key]['moran'][NAT].p_sim
...
for key in moran_ROI_nearest.keys():
    ...
    features = moran_ROI_nearest[key]['features']
    maxproj = moran_ROI_nearest[key]['maxproj']
    ROIs = moran_ROI_nearest[key]['ROIs']
    moran = moran_ROI_nearest[key]['moran'] 
    ...
    for NAT in NATs:
       ...
       plt.savefig('N:/axon2pmini/Article - Topography/Figures/Moran_ROI_nearest/'+key+'_'+NAT+'.svg', format = 'svg') 
