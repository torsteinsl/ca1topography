...
if __name__ == "__main__":
    ...
    #%% Already ran the script? Load the results instead and plot the data
    plotter = False  
        
    if plotter == True:
        
        results = pickle.load(open(session + '/decoding_topography_dict.pickle','rb'))
        means = results['means']
        meansShuffle = results['meansShuffle']
        ...
        # Decoding error from distance from reference place cell (per session)
        dfA, dfB, dfA2 = {}, {}, {}
        ...
        for sessionNo in range(nSessions-1):
            df = dfPlot[sessionNo]
            fig, ax = plt.subplots()
            sns.violinplot(data = df, ax = ax, palette = 'viridis')
            ax.set_xlabel('Anatomical distance (µm)')
            ax.set_ylabel('Decoding error (cm)')
            ax.set_title('Decoding error by distance from reference place cell')
            ax.spines[['top', 'right']].set_visible(False)
            plt.tight_layout()
        
        plt.savefig('N:/axon2pmini/Illustrations/img.svg', format = 'svg')  
        
        # Decoding error from distance from reference place cell (all sessions)
        dfPlotAll = {}
        for key in means.keys(): dfPlotAll[key] = means[key].flatten()
        dfPlotAll = pd.DataFrame.from_dict(dfPlotAll)
        
        fig, ax = plt.subplots()
        sns.violinplot(data = dfPlotAll, ax = ax, palette = 'viridis')
        ax.set_xlabel('Anatomical distance (µm)')
        ax.set_ylabel('Decoding error (cm)')
        ax.set_title('Decoding error by distance from reference cell')
        ax.spines[['top', 'right']].set_visible(False)
        plt.tight_layout()
        
        plt.savefig('N:/axon2pmini/Illustrations/img.svg', format = 'svg')  
    
        # Decoding error from nearby place cells and for shuffled data    
        dfPlotShuffleAll = {}
        for key in means.keys(): 
            dfPlotShuffleAll[key] = np.array(list(meansShuffle[key].values())).flatten()
        dfPlotShuffleAll = pd.DataFrame.from_dict(dfPlotShuffleAll)
        
        dfM1, dfM2 = dfPlotAll.melt(), dfPlotShuffleAll.melt()
        dfM1['Shuffle'], dfM2['Shuffle'] = 'Data', 'Shuffle' 
        
        dfMerge = pd.concat([dfM1, dfM2]) 
        
        fig, ax = plt.subplots()
        sns.violinplot(data = dfMerge, x = 'variable', y = 'value', hue = 'Shuffle', ax = ax, 
                        palette = 'viridis', split = True, rasterized = True)
        ax.set_xlabel('Anatomical distance (µm)')
        ax.set_ylabel('Decoding error (cm)')
        ...
        # See if the data is significatly different from the shuffled data
        stats = []
        for dist in distances: stats.append(ttest2(dfPlotAll[dist], dfPlotShuffleAll[dist]))
          
        # Number of cells within the anatomical distances (swarm, violin and box)
        dfCohort = {}
        for key in cohort.keys(): dfCohort[key] = cohort[key].flatten()
        dfCohort = pd.DataFrame.from_dict(dfCohort)
        
        fig, ax = plt.subplots()
        sns.swarmplot(data = dfCohort, ax = ax, size = 1.5, palette = 'viridis', dodge = False, rasterized = True)
        ax.set_title('Number of cells within given anatomical distance')
        ax.set_xlabel('Anatomical distance (µm)')
        ...
        fig, ax = plt.subplots()
        sns.violinplot(data = dfCohort, ax = ax, palette = 'viridis', scale = 'count', rasterized = True)
        ax.set_title('Number of cells within given anatomical distance')
        ax.set_xlabel('Anatomical distance (µm)')
        ax.set_ylabel('Number of cells')
        ax.spines[['top', 'right']].set_visible(False)
        ...
        fig, ax = plt.subplots()
        sns.boxplot(data = dfCohort, ax = ax, palette = 'viridis')
        ax.set_title('Number of cells within given anatomical distance')
        ax.set_xlabel('Anatomical distance (µm)')
        ax.set_ylabel('Number of cells')
        ax.spines[['top', 'right']].set_visible(False)
